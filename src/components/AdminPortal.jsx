...
export default function AdminPortal({ onNavigateToVoter }) {
  ...
  const loadAdminData = useCallback(async () => {
    try {
      ...
      // Calculate Stats
      const totalVotes = voteData ? voteData.reduce((acc, curr) => acc + (curr.votes_count || 0), 0) : 0;
      const totalCash = voteData ? voteData.reduce((acc, curr) => acc + (curr.amount || 0), 0) : 0;
  ...
  const exportTransactionsCsv = useCallback(() => {
    ...
    const header = ['date', 'reference', 'contestant', 'votes', 'amount', 'email', 'status'];
  ...
  const handleDeleteCategory = async (id) => {
    if (!window.confirm('Delete this category? This will delete all contestants in this category.')) return;
    try {
    ...
    return (
      <div className="app-container">
        ...
        <nav className="navbar">
          <a href="#" className="nav-brand" onClick={(e) => { e.preventDefault(); onNavigateToVoter(); }}>
            <ChevronLeft size={20} />
            <img src="/logo.jpg" alt="FASA logo" className="nav-logo" />
          </a>
          <button className="btn btn-secondary" onClick={onNavigateToVoter}>
            Voter Portal
          </button>
        </nav>
  ...
  return (
    <div className="dash-shell">
      ...
      <div className="dash-layout">
        {sidebarOpen && (
          <div className="dash-drawer-overlay" onClick={() => setSidebarOpen(false)} aria-hidden="true" />
        )}
        ...
        <aside className={`dash-sidebar ${sidebarOpen ? 'open' : ''}`}>
          ...
          <nav className="dash-nav">
            ...
            >
              <Layers size={18} /> Categories <span className="dash-nav-badge">{stats.categoriesCount}</span>
            </button>
            <button
              type="button"
              className={`dash-nav-item ${activeTab === 'contestants' ? 'active' : ''}`}
              onClick={() => { setActiveTab('contestants'); setSidebarOpen(false); }}
            >
              <Users size={18} /> Contestants <span className="dash-nav-badge">{stats.contestantsCount}</span>
            </button>
            <button
              type="button"
            ...
            >
              <History size={18} /> Transactions <span className="dash-nav-badge">{transactions.length}</span>
            </button>
          </nav>
        </aside>
        ...
        <main className="dash-main">
              ...
              <div className="dash-topbar-title">
                <strong>
                  {activeTab === 'dashboard' ? 'Overview' :
                    ...
                    activeTab === 'contestants' ? 'Contestants' :
                    activeTab === 'leaderboard' ? 'Leaderboards' :
                    'Transactions'}
                </strong>
                <span>
                  {activeTab === 'dashboard' ? 'Key metrics and recent activity' :
                    activeTab === 'categories' ? 'Organize contestants into groups' :
                    activeTab === 'contestants' ? 'Create contestants and assign categories' :
                    activeTab === 'leaderboard' ? 'Live rankings by category' :
                    'Payment and vote confirmations'}
                </span>
              </div>
            </div>
            <div className="dash-actions">
              <button type="button" className="btn btn-secondary" onClick={onNavigateToVoter}>
                <ChevronLeft size={16} /> Public site
              </button>
              <span className="dash-pill">{session?.user?.email || 'Admin'}</span>
          ...
          <div className="dash-content">
            {dataLoading && (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '20px', color: 'var(--accent-purple)' }}>
                <div className="spinner" style={{ width: '16px', height: '16px' }}></div>
                <span style={{ fontSize: '0.85rem' }}>Syncing data logs...</span>
              </div>
            )}
            ...
            {activeTab === 'dashboard' && (
              <div>
                <h2 style={{ fontSize: '1.8rem', marginBottom: '25px' }}>Performance Metrics</h2>
                <div className="stats-grid">
                  ...
                  <div className="stat-card glass-panel">
                    ...
                    <div className="stat-info">
                      ...
                      <span className="stat-value">₦{stats.totalCash.toLocaleString()}</span>
                    </div>
                  </div>
                  ...
                  <div className="stat-card glass-panel">
                    ...
                    <div className="stat-info">
                      ...
                      <span className="stat-value">{stats.contestantsCount}</span>
                    </div>
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '30px', marginTop: '30px' }} className="responsive-form-grid">
                  {/* Category Leaders summary */}
                  <div className="glass-panel" style={{ padding: '24px', background: 'rgba(13,9,38,0.2)' }}>
                    <h3 style={{ fontSize: '1.2rem', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <Crown size={20} color="#ec4899" fill="#ec4899" /> Category Leaders
                    </h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                      {categories.length === 0 ? (
                        ...
                        categories.map(cat => {
                          ...
                          return (
                            <div key={cat.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-glass)', paddingBottom: '10px' }}>
                              <div>
                                <h4 style={{ fontSize: '0.95rem' }}>{cat.name}</h4>
                                <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                                  {leader ? `Leader: ${leader.name}` : 'No contestants'}
                                </span>
                              </div>
                              {leader ? (
                                <div style={{ textAlign: 'right' }}>
                                  <span style={{ color: 'var(--accent-pink)', fontWeight: 700, fontSize: '0.9rem' }}>
                                    {leader.votes_count} votes
                                  </span>
                                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                                    ₦{(leader.votes_count * 100).toLocaleString()}
                                  </div>
                                </div>
                              ) : (
                  ...
                  <div className="glass-panel" style={{ padding: '24px', background: 'rgba(13,9,38,0.2)' }}>
                    <h3 style={{ fontSize: '1.2rem', marginBottom: '15px' }}>Recent Votes</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      {transactions.slice(0, 4).map(tx => (
                        <div key={tx.id} style={{ display: 'flex', justify: 'space-between', fontSize: '0.85rem', borderBottom: '1px solid var(--border-glass)', paddingBottom: '8px' }}>
                          <div>
                            <strong>{tx.contestants?.name || 'Deleted'}</strong>
                            <div style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>{tx.email}</div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                            <span style={{ color: 'var(--accent-purple)', fontWeight: 600 }}>+{tx.votes_count} Votes</span>
                            <div style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>₦{tx.amount.toLocaleString()}</div>
                          </div>
                        </div>
                      ))}
                      {transactions.length === 0 && (
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>No votes cast yet.</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            {activeTab === 'categories' && (
              <div>
                ...
                <form onSubmit={handleCategorySubmit} className="glass-panel" style={{ padding: '20px', marginBottom: '30px', background: 'rgba(13,9,38,0.3)' }}>
                  <h3 style={{ fontSize: '1.1rem', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <PlusCircle size={18} className="text-accent-purple" /> 
                    {editingCategory ? `Modify: ${editingCategory.name}` : 'Add New Category'}
                  </h3>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }} className="responsive-form-grid">
                ...
                <h3 style={{ fontSize: '1.3rem', marginBottom: '15px' }}>All Categories</h3>
                {categories.length === 0 ? (
            ...
            {activeTab === 'contestants' && (
              <div>
                <h2 style={{ fontSize: '1.8rem', marginBottom: '25px' }}>
                  {editingContestant ? `Edit Contestant: ${editingContestant.name}` : 'Contestant Management'}
                </h2>
                
                <form onSubmit={handleContestantSubmit} className="glass-panel" style={{ padding: '20px', marginBottom: '30px', background: 'rgba(13,9,38,0.3)' }}>
                  <h3 style={{ fontSize: '1.1rem', marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <PlusCircle size={18} className="text-accent-purple" /> 
                    {editingContestant ? 'Modify Details' : 'Upload New Contestant'}
                  </h3>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '15px' }} className="responsive-form-grid">
                    <div className="form-group">
                      <label>Option A: Image URL Link</label>
                      <div style={{ position: 'relative' }}>
                        <ImageIcon style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} size={16} />
                        <input 
                          type="url" 
                          className="input-control" 
                          ...
                          onChange={(e) => setContestantImageUrl(e.target.value)}
                    ...
                    <div className="form-group">
                      ...
                      <input 
                        ...
                        onChange={(e) => setContestantImageFile(e.target.files[0])}
                  ...
                  <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                    <button type="submit" className="btn btn-primary" disabled={submittingContestant}>
                      {submittingContestant ? 'Saving details...' : editingContestant ? 'Save Changes' : 'Upload Contestant'}
                    </button>
                    {editingContestant && (
                      <button 
                        type="button" 
                ...
                <h3 style={{ fontSize: '1.3rem', marginBottom: '15px' }}>All Contestants</h3>
                {contestants.length === 0 ? (
                  <p style={{ color: 'var(--text-muted)' }}>No contestants found. Add categories first, then add contestants.</p>
                ) : (
                  <>
                        ...
                        {contestants
                          .filter(con => {
                            ...
                            const matchesCategory =
                              ...
                              String(con.category_id) === String(contestantCategoryFilter);
                          ...
                          <tr key={con.id}>
                            <td>
                              <div style={{ width: '40px', height: '40px', borderRadius: '8px', overflow: 'hidden', background: 'rgba(0,0,0,0.3)' }}>
                                <img src={CONTESTANT_LOGO} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                              </div>
                            </td>
                            <td><strong>{con.name}</strong></td>
                            <td>
                              <span style={{ background: 'rgba(139,92,246,0.1)', color: 'var(--accent-purple)', padding: '4px 8px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 500 }}>
                                {con.categories?.name || 'Unassigned'}
                              </span>
                            </td>
                            <td><strong style={{ color: 'var(--accent-pink)' }}>{con.votes_count.toLocaleString()}</strong></td>
                            <td><strong>₦{(con.votes_count * 100).toLocaleString()}</strong></td>
                            <td style={{ textAlign: 'right', display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                              <button 
                                className="btn btn-secondary" 
                                style={{ padding: '6px 12px', fontSize: '0.8rem' }}
                                onClick={() => handleStartEditContestant(con)}
                              >
                                <Edit3 size={13} /> Edit
                              </button>
                              <button 
            ...
            {activeTab === 'leaderboard' && (
              <div>
                <h2 style={{ fontSize: '1.8rem', marginBottom: '10px' }}>Category Leaderboards</h2>
                <p style={{ color: 'var(--text-muted)', marginBottom: '30px' }}>
                  Auditing live rankings per category. Ratio: ₦100 = 1 Vote (₦1,000 = 10 Votes).
                </p>

                {categories.length === 0 ? (
                  <p style={{ color: 'var(--text-muted)' }}>Please configure categories first to view standings.</p>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
                    {categories.map(cat => {
                      ...
                      return (
                        <div key={cat.id} className="glass-panel" style={{ padding: '24px', background: 'rgba(13,9,38,0.2)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-glass)', paddingBottom: '12px', marginBottom: '15px' }}>
                            <div>
                              <h3 style={{ fontSize: '1.3rem', color: 'var(--text-white)' }}>{cat.name}</h3>
                              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{cat.description || 'No category description'}</p>
                            </div>
                            <span style={{ fontSize: '0.8rem', background: 'rgba(59,130,246,0.1)', color: 'var(--accent-blue)', padding: '5px 12px', borderRadius: '50px', fontWeight: 600 }}>
                              {standings.length} {standings.length === 1 ? 'Contestant' : 'Contestants'}
                            </span>
                          </div>
                                ...
                                return (
                                  <div 
                                    ...
                                    style={{ 
                                      ...
                                      padding: '12px 18px', 
                                      background: isLeader ? 'rgba(236,72,153,0.06)' : 'rgba(255,255,255,0.01)', 
                                      border: isLeader ? '1px solid rgba(236,72,153,0.2)' : '1px solid var(--border-glass)',
                                      borderRadius: '12px' 
                                    }}
                                  >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                      <span style={{ fontStyle: 'italic', fontWeight: 700, width: '20px', color: index === 0 ? 'var(--accent-pink)' : 'var(--text-muted)' }}>
                                        #{index + 1}
                                      </span>
                                      <div style={{ width: '35px', height: '35px', borderRadius: '50%', overflow: 'hidden', background: 'var(--bg-deep)' }}>
                                        <img src={CONTESTANT_LOGO} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                      </div>
                                      <div>
                                        <strong style={{ fontSize: '0.95rem' }}>{con.name}</strong>
                                        {isLeader && (
                                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', marginLeft: '10px', fontSize: '0.75rem', color: 'var(--accent-pink)', background: 'rgba(236,72,153,0.1)', padding: '2px 8px', borderRadius: '50px', fontWeight: 600 }}>
                                            <Crown size={10} fill="var(--accent-pink)" /> Category Leader
                                          </span>
                                        )}
                                      </div>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                                      <div style={{ textAlign: 'right' }}>
                                        <span style={{ fontWeight: 700, color: index === 0 ? 'var(--accent-pink)' : 'var(--text-white)' }}>
                                          {con.votes_count.toLocaleString()}
                                        </span>
                                        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}> {con.votes_count === 1 ? 'vote' : 'votes'}</span>
                                      </div>
                                      <div style={{ minWidth: '80px', textAlign: 'right' }}>
                                        <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>₦{(con.votes_count * 100).toLocaleString()}</span>
                                      </div>
                                    </div>
                                  </div>
                                );
            ...
            {activeTab === 'transactions' && (
              <div>
                <h2 style={{ fontSize: '1.8rem', marginBottom: '25px' }}>Transaction History Logs</h2>
                      ...
                      <tbody>
                        {transactions.map(tx => (
                          <tr key={tx.id}>
                            ...
                            <td style={{ fontFamily: 'monospace', fontSize: '0.8rem' }}>{tx.reference}</td>
                            <td><strong>{tx.contestants?.name || 'Deleted Contestant'}</strong></td>
